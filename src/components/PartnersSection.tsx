  ...
  return (
    <section id="partners" className="py-24 bg-[#f7f2e8] relative border-t border-[#e6ddcc]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {filteredPartners.map(partner => {
            ...
            const baseEducation = Array.isArray(partner.education) ? partner.education.filter(Boolean) : [];
            const educationList = baseEducation.map((edu, idx) => {
              if (lang === 'tr' && partner.educationTr?.[idx]?.trim()) return partner.educationTr[idx];
              if (lang === 'en' && partner.educationEn?.[idx]?.trim()) return partner.educationEn[idx];
              return edu;
            });
            ...
            return (
              <div
                key={partner.id}
                className="group rounded-2xl bg-white border border-[#e6ddcc] hover:border-[#b38a38] transition-all duration-300 overflow-hidden flex flex-col justify-between hover:-translate-y-1.5 shadow-md hover:shadow-xl font-cards-custom"
              >
                ...
                <div className="p-5 flex flex-col flex-grow justify-between space-y-3">
                  <div className="space-y-2">
                    ...
                    {barAdmission && (
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#b38a38]/10 border border-[#b38a38]/30 text-[11px] font-bold text-[#87641d]">
                        <ShieldCheck className="w-3.5 h-3.5 flex-shrink-0 text-[#b38a38]" />
                    ...
                    <p className="text-xs font-bold text-[#87641d]">
                      {title}
                    </p>

                    {/* Primary Specialty */}
                    {specialty && (
                      <div className="flex items-start gap-1.5 text-xs font-semibold text-[#2c261e] bg-[#f7f2e8] px-2.5 py-1.5 rounded-lg border border-[#e6ddcc]">
                        <Scale className="w-3.5 h-3.5 text-[#87641d] flex-shrink-0 mt-0.5" />
                        <span>{specialty}</span>
                      </div>
                    )}

                    {/* Education & Professional Accreditations (e.g. محكم معتمد من نقابة المحامين في سورية) */}
                    {educationList.length > 0 && (
                      <div className="pt-1 space-y-1.5">
                        <span className="text-[11px] font-bold text-[#87641d] flex items-center gap-1">
                        ...
                        <div className="space-y-1">
                          {educationList.map((edu, idx) => (
                            <div
                              key={idx}
                              className="flex items-start gap-1.5 text-[11px] font-medium text-[#2c261e] bg-[#fbf8f2] px-2.5 py-1.5 rounded-lg border border-[#e6ddcc]"
                            >
                    ...
                    <p className="text-xs text-[#5c5343] line-clamp-3 leading-relaxed pt-1 whitespace-pre-line">
                      {bio}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-[#5c5343] bg-[#f7f2e8]/70 px-3 py-1.5 rounded-lg border border-[#e6ddcc]/80">
                    ...
                    <div className="pt-2 border-t border-[#e6ddcc] flex items-center justify-between">
                      <button
                        onClick={() => setSelectedPartner(partner)}
                        className="text-xs font-bold text-[#87641d] hover:text-[#181512] flex items-center gap-1 transition cursor-pointer"
                      >
                        <span>{lang === 'ar' ? 'السيرة والشهادات الكاملة' : lang === 'tr' ? 'Özgeçmiş ve Yetkinlikler' : 'Full Profile'}</span>
      ...
      {selectedPartner && (() => {
        ...
        const baseEdu = Array.isArray(activePartner.education) ? activePartner.education.filter(Boolean) : [];
        const modalEduList = baseEdu.map((edu, idx) => {
          if (lang === 'tr' && activePartner.educationTr?.[idx]?.trim()) return activePartner.educationTr[idx];
          if (lang === 'en' && activePartner.educationEn?.[idx]?.trim()) return activePartner.educationEn[idx];
          return edu;
        });
